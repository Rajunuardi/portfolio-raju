/* ================================
   ...
   (FULL UPDATE)
   - View Live project buka Lightbox (besar & jelas)
   - Ada tombol "Kembali" di Lightbox (tanpa refresh)
   ================================ */
...
window.addEventListener('load', () => {
  window.scrollTo(0, 0);
  // Bersihkan hash kalau ada (supaya reload tidak lompat ke anchor)
  if (location.hash) {
...
(() => {
  ...
  if (saved && modes.includes(saved)){
    ...
    toggle && (toggle.textContent = saved==="bg-neutral"?"🌞":saved==="bg-blue"?"🌊":"🌙");
  } else {
    if(!modes.some(m=>body.classList.contains(m))){
...
function animateBars(){
  document.querySelectorAll(".skill-bar .fill").forEach((bar,i)=>{
    const p = parseInt(bar.getAttribute("data-percent"),10);
    let c=0;
    setTimeout(()=>{
      const t = setInterval(()=>{
        if(c>=p) clearInterval(t);
          ...
          c++;
          bar.style.width = c+"%";
          bar.querySelector("span").textContent = c+"%";
        }
      },15);
    }, i*150);
  });
    ...
    setTimeout(()=>{
      const t=setInterval(()=>{
        if(c>=p) clearInterval(t);
          ...
          c++;
          prog.style.strokeDashoffset = C - (c/100)*C;
          num.textContent = c+"%";
        }
      },20);
    }, i*200);
...
/* =========================================================
   LIGHTBOX (UPDATED)
   - Bisa untuk gambar cert/gallery + gambar project "View Live"
   - Tambah tombol KEMBALI (tanpa refresh)
   - Jika user klik View Live, tombol kembali muncul
   ========================================================= */
...
// gallery default (cert + card image)
const gallery = document.querySelectorAll(".card img, .certificate-img");
let idx = 0;

// util buat bikin tombol kembali + wrapper (sekali saja)
function ensureLightboxUI(){
  if (!lb) return;

  // bikin wrapper inner supaya layout rapi (kalau belum ada)
  // NOTE: HTML kamu sekarang: <div id="lightbox"> <span class="close"> ... <img ...> <div id="caption"></div> </div>
  // CSS yang aku kirim support .lb-inner, jadi kita bungkus otomatis.
  let inner = lb.querySelector(".lb-inner");
  if (!inner) {
    inner = document.createElement("div");
    inner.className = "lb-inner";

    // pindahkan img + caption ke inner
    if (lbImg) inner.appendChild(lbImg);
    if (lbCap) inner.appendChild(lbCap);

    // taruh inner setelah tombol close (biar close tetap di atas)
    const closeEl = lb.querySelector(".close");
    if (closeEl && closeEl.nextSibling) {
      lb.insertBefore(inner, closeEl.nextSibling);
    } else {
      lb.appendChild(inner);
    }
  }

  // tombol kembali (buat View Live project)
  let backBtn = lb.querySelector(".lb-back");
  if (!backBtn) {
  ...
  return { inner, backBtn };
}

function openLightbox({ src, alt = "", caption = "", showBack = false }){
  if (!lb || !lbImg) return;
  const ui = ensureLightboxUI();

  lbImg.src = src;
  lbImg.alt = alt || caption || "";
  if (lbCap) lbCap.textContent = caption || alt || "";

  // tampilkan tombol kembali hanya untuk View Live
  if (ui?.backBtn) ui.backBtn.style.display = showBack ? "inline-flex" : "none";
...
function closeLightbox(){
  ...
  // optional: bersihkan src biar ga berat
  if (lbImg) lbImg.src = "";
  if (lbCap) lbCap.textContent = "";
}

function showLB(i){
  const imgs = Array.from(gallery);
  ...
  openLightbox({
    ...
    alt: imgs[i].alt || "",
    caption: imgs[i].alt || "",
    showBack: false // gallery biasa cukup close X
  });
}

// click pada gambar cert/gallery
...
lb?.addEventListener("click",(e)=>{
  // kalau klik background (bukan img / caption / tombol)
...
/* =========================================================
   View Live (Project) -> buka gambar dashboard di Lightbox
   HTML:
   <a class="media-btn view-dashboard" href="images/project1.png"
      data-caption="Student Achievement Ranking (ELECTRE) – Dashboard Preview">View Live</a>
   ========================================================= */
document.querySelectorAll(".view-dashboard").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    // buka lightbox mode "View Live" => tombol kembali muncul
    openLightbox({
      src,
      alt: cap,
      caption: cap,
